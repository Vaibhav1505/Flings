...
class Variant{
    ...
    getProductId(){return this.obj.thing_id;}
    ...
    updateField(field,value,cb){
        $.ajax({
            ...
            method:'PUT',
            data:{_id:this.getVariantId(),thing_id:this.getProductId(),field:field,value:value},
    ...
    deleteVariant(cb){
        $.ajax({
            url:'/admin/products/'+this.getProductId()+"/variants/"+this.getVariantId(),
            method:'DELETE',
            data:{},
            success:function(data,textStatus,xhr){
                cb(xhr,textStatus);
    ...
    getView(){
        ...
        var d2e = __cd('img',null,{src:"/admin/products/media/"+this.getImage()+"_small"},null);
...
class Media{
    ...
    getThumbImgSrc(){
        return location.protocol+'//'+location.host+"/products/media/"+this.getMediaId()+'_small';
    }

    getLargeImgSrc(){
        return location.protocol+'//'+location.host+"/products/media/"+this.getMediaId();

    }

    setAsDefault(cb){
        $.ajax({
            url:'/admin/products/'+this.getProductId(),
            type:"PUT",
            data:{_id:this.getProductId(),field:'image',value:this.getMediaId()},
            success:function(data,textStatus,xhr){
                cb(xhr,textStatus);
            },
            error:function(xhr,textStatus,err){
    ...
    removeMedia(cb){
        $.ajax({
           ...
           method:'put',
            data:{_id:this.getMediaId(),thing_id:this.getProductId()},
            success:function(data,textStatus,xhr){
                cb(xhr,textStatus);
            },
            error:function(xhr,textStatus,err){
    ...
    getView(){
        ...
        e.addEventListener('click',function (){
            new Modal('confirm','Are you sure want to set this media as default/main for this product?',function () {
...
function updateProduct(_id,domElement,beforeSend,complete,cb){
    $.ajax({
...
function addSaveListeners(){
    ...
    for(var i=0;i<arr.length;i++){
        ...
        save.addEventListener('click',function(){
           updateProduct(getLastParamUrl(),field,beforeSend,complete,function(xhr,textStatus){
               if(xhr.status==201){
                   save.hidden = true;
    ...
    document.querySelector('#save-variants').addEventListener('click',function(){
       ...
       $.ajax({
           url:'/admin/products/'+getLastParamUrl()+"/options",
           method:'post',
